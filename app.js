...
for (var i = 0; i < productList.length; i++) {
  var section1 = document.getElementById("section_id");
  var div1 = document.createElement("div");
  var span1 = document.createElement("span");
  var span2 = document.createElement("span");
  var clothSecContainer = document.createElement("span");
  if (i == 0) {
    clothSecContainer.className = "clothSection_text";
    clothSecContainer.innerHTML = "Clothing for Men and Women";
    section1.appendChild(clothSecContainer);
  }
  if (i == 5) {
    clothSecContainer.className = "accessoriesSection_text";
    clothSecContainer.innerHTML = "Accessories for Men and Women<br>";
    section1.appendChild(clothSecContainer);
  }
  span1.className = "img_class";
  span2.className = "content_class";
  div1.appendChild(span1);
  div1.appendChild(span2);
  section1.appendChild(div1);
  for (var k in productList[i]) {
    if (k == "name") {
      var textContent = productList[i][k];
      var names = document.createElement("h1");
      var nameContent = document.createTextNode(textContent);
      names.appendChild(nameContent);
      span2.appendChild(names);
    }
    if (k == "preview") {
      var images = productList[i][k];
      span1.innerHTML = "<img src=" + images + " />";
    }
    if (k == "brand") {
      var create_h1 = document.createElement("h3");
      span2.appendChild(create_h1);
      create_h1.innerHTML = productList[i][k];
    }
    if (k == "price") {
      var create_h1 = document.createElement("h2");
      span2.appendChild(create_h1);
      create_h1.innerHTML = productList[i][k];
    }
  }
}
// AUTHOR: GANESH M NAIK :966321070
